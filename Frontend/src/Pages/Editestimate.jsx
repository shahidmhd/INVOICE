// import React from 'react'
// import Sidebar from '../Components/Sidebar/Sidebar'
// import Estimateedit from '../Components/Editestimate/Estimateedit'

// const Editestimate = () => {
//   return (
//     <>
//     <div style={{ display: 'flex' }}>
//       <div style={{ flex: '0 0 auto' }}>
//         <Sidebar />
//       </div>
//       <div style={{ flex: '1', overflowY: 'scroll' }}>
//         {<Estimateedit/>}
//       </div>
//     </div>

//   </>
//   )
// }

// export default Editestimate

import React, { useEffect, useState } from 'react'
import Sidebar from '../Components/Sidebar/Sidebar'
import { getallcompanies } from '../apicalls/Company'
import { getallServices } from '../apicalls/Service'
import { useParams } from 'react-router-dom'
import Estimateedit from '../Components/Editestimate/Estimateedit'
import { getselectedEstimate } from '../apicalls/Estimate'

const Editestimate = () => {
  const { id } = useParams();
  const [invoiceData, setInvoiceData] = useState(null);
  const [companydetails, setcompanydetails] = useState([]);
  const [servicedetails, setservicedetails] = useState([]);

  useEffect(() => {
    const fetchInvoiceData = async () => {
      try {
        const response = await getselectedEstimate(id);
        setInvoiceData(response.Data);
      } catch (error) {
        console.log(error);
      }
    };
    fetchInvoiceData();
  }, [id]);

  useEffect(() => {
    getcomapanydata()
    getServicedata()
  }, [])


  const getcomapanydata = async () => {
    const response = await getallcompanies()
    setcompanydetails(response.Data)
  }

  const getServicedata = async () => {
    const responseservice = await getallServices()
    setservicedetails(responseservice.Data)
  }



  return (
    <>
      <div style={{ display: 'flex' }}>
        <div style={{ flex: '0 0 auto' }}>
          <Sidebar />
        </div>
        <div style={{ flex: '1', overflowY: 'scroll' }}>
          {invoiceData && <Estimateedit companydetails={companydetails} servicedetails={servicedetails} invoiceData={invoiceData} />}
        </div>
      </div>

    </>
  )
}

export default Editestimate

